...
exports.getCreate = (req, res, next) => {
  let sqlRole = knex("Role").select("Role.id", "Role.name").toString();
...
exports.create = (req, res, next) => {
  ...
  userAccount.password = bcrypt.hashSync(
...
exports.update = (req, res, next) => {
  ...
  UserAccount.update(userAccount, { where: { id: req.params.id } })
    .then(async () => {
      await (async () => {
...
exports.delete = (req, res, next) => {
  UserAccount.destroy({ where: { id: req.params.id } })
    .then(() => {
      res.end();
    })
    .catch(next);
